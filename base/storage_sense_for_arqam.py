...
class Saver(object):
        ...
        try:
            if os.path.exists(pth):
                ...
                if self.file_extension=='.csv':
                    ...
                    self.file=pd.read_json(open(pth,'r'),dtype=False)
        ...
        if len(os.listdir(self.block_address))>0:
            for obj in os.listdir(self.block_address):
                ...
                if os.path.isdir(os.path.join(block_address,obj)):
                    self.block={'address':'tasks.'+str(uuid)+'.outputs.'+str(obj)}
                    self.load_deep_stuff()
    ...
    def get_payload_data(self,service,data_point):
        ...
        return False
    def get_next_cursor(self,identifier,service,data_point,block=False):
