...
class ConcurrentRunner:
    ...
    def run_tasks_concurrently(self):
        ...
        running_tasks = []
        while True:
            while self.task_index < len(self.task_list) or running_tasks:
                # Check if we can start a new task
                if self.task_index < len(self.task_list) and len(running_tasks) < self.max_concurrent_tasks:
                    task_args = self.task_list[self.task_index]
